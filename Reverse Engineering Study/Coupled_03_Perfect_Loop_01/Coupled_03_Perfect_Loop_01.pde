...
float[] initField(int vals, float min, float max) {
  ...
  for(int i = 0; i < vals; i++){
    // to get a nice circle, the circumference is divided up into vals segments
    // to fit vals segments into a circle,
    float sampleA = i*TWO_PI/vals;
    float sampleX = 1.05*cos(sampleA); 
    float sampleY = 1.05*sin(sampleA);
