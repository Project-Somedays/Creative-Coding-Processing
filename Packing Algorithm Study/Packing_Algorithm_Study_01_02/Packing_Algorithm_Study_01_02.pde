...
void draw() {
  ...
  for (int i = 0; i < bubbles.size(); i++) {
    for (int j = 0; j < bubbles.size(); j++) {
      ...
      if (PVector.dist(bubbles.get(i).pos, bubbles.get(j).pos) < bubbles.get(i).r + bubbles.get(j).r + 2*strWeight) {
...
void spawnBubble() {
  ...
  while (attempts < 100) {
    ...
    for (int i = 0; i < bubbles.size(); i++) {
      ...
      if (dist(x, y, bubbles.get(i).pos.x, bubbles.get(i).pos.y) < bubbles.get(i).r*1.05) {
        attempts ++;
