...
color[] colours = {#f8f4e3,#ffffff,#eed7c5};
float[][] flowField;
float step = 0.0005;
float yOff, xOff;
...
//color bg = #48cae4;
color bg = #abc4ff;
float circleOverlapFraction = 0.8;
...
void draw() {
  ...
  for (int i = 0; i < swarm.size(); i++) {
    for (int j = 0; j < swarm.size(); j++) {
      ...
      if (PVector.dist(swarm.get(i).pos, swarm.get(j).pos) <= circleOverlapFraction*(swarm.get(i).r + swarm.get(j).r)) {
